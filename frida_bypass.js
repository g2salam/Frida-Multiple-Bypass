    ...
    try {
        Interceptor.attach(Module.findExportByName(null, "android_getCpuFamily"), {
            onLeave: function(retval) {
                ...
                if ([2, 5].indexOf(retval.toInt32()) > -1) {
...
setTimeout(function() {
    ...
    Java.perform(function() {
        ...
        PackageManager.getPackageInfo.overload(
            ...
            var shouldFakePackage = ROOTmanagementApp.indexOf(pname) > -1;
        ...
        */
        ...
        ProcessBuilder.start.implementation = function() {
            ...
            for (var i = 0; i < cmd.size(); i = i + 1) {
                ...
                if (tmp_cmd == "su" || tmp_cmd.endsWith("/su")) {
                    shouldBypassSu = true;
                }
            }
...
setTimeout(function() {
    Java.perform(function() {
        ...
        try {
            const AndroidWebViewClient = Java.use("android.webkit.WebViewClient");
            AndroidWebViewClient.onReceivedSslError.overload(
        ...
        try {
            const CertificateTransparencyHostnameVerifier = Java.use(
                "com.appmattus.certificatetransparency.internal.verifier.CertificateTransparencyHostnameVerifier"
            );
            CertificateTransparencyHostnameVerifier["verify"].overload(
                "java.lang.String",
                "javax.net.ssl.SSLSession"
            ).implementation = function(host, session) {
                console.log(
                    "  --> Bypassing Appmattus (CertificateTransparencyHostnameVerifier): " + host
                );
                return true;
            };
            console.log("[+] Appmattus (CertificateTransparencyHostnameVerifier)");
        } catch (err) {
            console.log("[ ] Appmattus (CertificateTransparencyHostnameVerifier)");
        }
